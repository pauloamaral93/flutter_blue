...
//                 }
			...
			//gattServer = device.connectGatt(registrar.activity(), false, mGattCallback);
                    gattServer = device.connectGatt(registrar.activity(), options.getAndroidAutoConnect(), mGattCallback, BluetoothDevice.TRANSPORT_LE , BluetoothDevice.PHY_LE_CODED);
                } else {
			 			 try 
{
    Thread.sleep(500);
...
catch(InterruptedException e)
{
     // this part is executed when an exception (in this example InterruptedException) occurs
				...
				for(BluetoothGattService s : gatt2.getServices()) {
				    p.addServices(ProtoMaker.from(gatt2.getDevice(), s, gatt2));
				}
